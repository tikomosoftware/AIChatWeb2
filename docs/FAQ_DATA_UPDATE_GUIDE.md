...
- ベクトル次元数が **384** であること（`all-MiniLM-L6-v2` モデルの場合）
...
現在のデフォルト: `sentence-transformers/all-MiniLM-L6-v2`（384次元）
...
`all-MiniLM-L6-v2` は英語特化の軽量モデルのため、日本語テキスト同士の類似度スコアは低めに出ます。
